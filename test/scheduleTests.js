...
describe('Australia - do not send on weekends', function () {
  ...
  // before day
  it('Start: Friday 05:27, 1 min delay, maxDelay: 20 min. Schedules Friday at 9:01', function () {
    assert.equal(scheduler.getTime("09:00", "18:00", 1535657223, 60000, undefined, undefined, 1200000, 0, 0, "Australia/Sydney"), 1535670063)
  })
  // during day
  it('Start: Thursday 11:34, 1 min delay, maxDelay: 20 min. Schedules Thursday at 1135', function () {
    assert.equal(scheduler.getTime("09:00", "18:00", 1536197640, 60000, undefined, undefined, 1200000, 0, 0, "Australia/Sydney"), 1536197700)
  })
  // after late cutoff
  it('Start: Wednesday 20:35, 10 min delay, maxDelay: 20 min. Schedules Thursday at 09:10', function () {
  ...
  // DAY 0 Weekend
  // before day
  it('Start: Saturday 07:35, 13 min delay, maxDelay: 1 hour Schedules Monday at 9:13', function () {
    assert.equal(scheduler.getTime("09:00", "18:00", 1536356114, 780000, undefined, undefined, 3600000, 0, 0, "Australia/Sydney"), 1536361994)
  })
  // during day
  it('Start: Sunday 15:02, 42 min delay, maxDelay: 1 hour Schedules Sunday at 15:44', function () {
    assert.equal(scheduler.getTime("09:00", "18:00", 1536469320, 2520000, undefined, undefined, 3600000, 0, 0, "Australia/Sydney"), 1536471840)
  })
  // after late cutoff
  it('Start: Sunday 22:20, 29 min delay, maxDelay: 1 hour Schedules Monday at 09:29', function () {
  ...
  // before day
  it('Start: Saturday 05:29, maxDelay: 1 hour Schedules Sunday at 12:23', function () {
    assert.equal(scheduler.getTime("09:00", "18:00", 1536953340, undefined, 23, 12, 3600000, 1, 0, "Australia/Sydney"), 1537064580)
  })
  // during day
  it('Start: Saturday 10:45, maxDelay: 1 hour Schedules Sunday at 11:00', function () {
    assert.equal(scheduler.getTime("09:00", "18:00", 1536972300, undefined, 00, 11, 3600000, 1, 0, "Australia/Sydney"), 1537059600)
  })
  // after late cutoff
  it('Start: Sunday 22:03, maxDelay: 1 hour Schedules Tuesday at 16:20', function () {
    assert.equal(scheduler.getTime("09:00", "18:00", 1537099380, undefined, 20, 16, 3600000, 1, 0, "Australia/Sydney"), 1537251600)
  })
})
// US - send on weekends
